...
def index():
    df2 = playstore.copy()
    trial =  df2['Category'].value_counts()
    ...
    print(top_category.loc[0,'Jumlah'])
    stats = {
        'most_categories' :top_category['Category'].iloc[0],
        'total': 1972,#top_category.loc[0,'Jumlah'],
        'rev_table' : df2[['Category','App','Reviews','Rating']].sort_values(by='Reviews',ascending=False).head(10).reset_index(drop=True).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    
    ## Bar Plot
    cat_order = df2.groupby('Category').agg({
    'App' : 'count'
        }).rename({'App':'Total1'}, axis=1).sort_values(by='Total1',ascending=False).reset_index().head()
    ...
    cont_int = pd.crosstab(df2['Content Rating'],
                           df2['Genres'],
                           values = df2['Installs'],
                           aggfunc='sum')
    con_int_plot = cont_int.stack().reset_index(name='Total Installs')
